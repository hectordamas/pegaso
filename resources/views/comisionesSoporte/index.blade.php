...
@section('metadata')
    <title>Comisiones Soporte - {{ env('APP_NAME') }}</title>
@endsection
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    ...
                    <span>Filtra y analiza las comisiones de la Gerencia de Soporte Técnico

                    </span>
                </div>
                <div class="card-block">
                    <form class="row" action="{{ url('comisiones-soporte') }}">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <h5>Comisiones Gerencia de Soporte Técnico</h5>

                </div>
                <div class="card-block">
                    <table class="table table-striped" id="ComisionesSoporte">
...
@section('scripts')
    <script>
        ...
        function getComisionesData() {
            if ($.fn.DataTable.isDataTable('#ComisionesSoporte')) {
                $('#ComisionesSoporte').dataTable().fnClearTable();
                $('#ComisionesSoporte').dataTable().fnDestroy();
            }

            $('#ComisionesSoporte').DataTable({
                order: [
                    [0, 'desc']
                ],
                "bDeferRender": true,
                "bProcessing": true,
                "sAjaxSource": "{{ route('comisionesSoporte.getComisionesData') }}",
        ...
        $(document).on('change', '.check-admin, .check-manager', function() {
            ...
            $.post(`/comisionesSoporte/safact/${id}/check`, data, function(res) {
                if (res.success) {
